...
def get_data_collator(tokenizer):
    # Updated to DataCollatorForLanguageModeling for training with paragraphs
    collator = DataCollatorForLanguageModeling(
        tokenizer=tokenizer,
        mlm=False  # No masked language modeling, standard causal LM training
    )
    return collator
...
if __name__ == '__main__':
    ...
    MODEL_NAME = "EZStorytellingEditsSFT_fd_1k"
    ...
    # Load dataset
    train_dataset = load_data(f'ChaiML/EZ_sft_fd_1k')
    ...
    splits = train_dataset['text'][0].split('\n')
    prompt = '\n'.join(splits[:-1])
    expected_response = splits[-1]
    ...
    submitter = ModelSubmitter('CR_5a47df3092764a8d9dfae7da900291ea', verbose=True)
